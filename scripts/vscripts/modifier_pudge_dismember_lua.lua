...
function modifier_pudge_dismember_lua:OnCreated( kv )
    ...
    local attacktime = self:GetCaster():GetBaseAttackTime() / self:GetCaster():GetAttackSpeed()
    self.tick_rate = self.tick_rate/attacktime
...
function modifier_pudge_dismember_lua:OnIntervalThink()
	if IsServer() then
        local currentdmg = math.floor(0.5 * self:GetCaster():GetAverageTrueAttackDamage(self:GetCaster()))
		local damage = {
		...
		self:GetCaster():Heal( currentdmg, self:GetAbility() )
	end
end
